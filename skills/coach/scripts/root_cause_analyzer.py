...
class RootCauseAnalyzer:
    ...
    def _diff_commands(self, cmd1: str, cmd2: str) -> Optional[Dict]:
        ...
        return {"type": "unknown_change", "details": {"from": primary1, "to": primary2}}
...
def test_compound_command_parsing():
    ...
    tests = [
        ...
        ("cmd1; cmd2; cmd3", ["cmd1", "cmd2", "cmd3"]),
        (
            "cd /path && ./vendor/bin/phpunit --testsuite=unit",
            ["cd /path", "./vendor/bin/phpunit --testsuite=unit"],
        ),
    ...
    tests = [
        ...
        (
            "cd /home/foo && ./vendor/bin/phpunit --testsuite=unit",
        ...
        (
            "COMPOSE_PROFILES=e2e docker compose run --rm app",
    ...
    tests = [
        ...
        (
            "COMPOSE_PROFILES=e2e docker compose run",
            "docker compose",
        ),  # Env var stripped
...
def test_diff_uses_primary_command():
    ...
    assert "--testsuite=functional" in diff["details"]["added"], (
        f"Expected --testsuite=functional in added, got {diff['details']}"
    )
...
def main():
    ...
    if args.context_file:
        ...
        analyzer.add_command("gh pr merge 123 --auto", exit_code=0, stderr="")
