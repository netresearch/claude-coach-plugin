                        ...
                        if isinstance(item, dict):
                            if item.get("type") == "text":
                                ...
                                item["is_failure"] = (
                                    ...
                                    or "exit code 1" in result_text
                                    or "failed" in result_text[:100]
                                    or "command not found" in result_text
                                )
...
class CandidateAggregator:
    ...
    def extract_candidate_from_failure(self, events: List[Dict]) -> List[Dict]:
        ...
        for base_cmd, cmd_events in command_groups.items():
            ...
            event = cmd_events[-1]
            content = json.loads(event.get("content", "{}"))
            json.loads(event.get("context", "{}"))

            command = content.get("command", "")
            stderr = content.get("stderr_preview", "")
            content.get("exit_code", 1)
    ...
    def _extract_failure_pattern(
        self, command: str, stderr: str, matches: List[str], failure_count: int
    ) -> Tuple[str, str, str]:
        ...
        if failure_count >= 2:
            return (
                ...
                "investigate root cause before retrying; check prerequisites and error messages",
                "rule",
            )
    ...
    def extract_candidate_from_verification(self, events: List[Dict]) -> List[Dict]:
        ...
        for action, events_contents in action_events.items():
            ...
            candidate = {
                ...
                "candidate_type": "checklist",
                "trigger": "before marking a task as complete",
        ...
        if self.transcript_analyzer:
            ...
            if "error" not in transcript_analysis:
                ...
                if verbose:
                    ...
                    print(
                        f"  Found {summary.get('correction_count', 0)} corrections, "
    ...
    def _update_ledger(self, candidates: List[Dict]):
        ...
        try:
            ...
            repo_id = (
                hashlib.sha256(result.stdout.strip().encode()).hexdigest()[:16]
                if result.returncode == 0
                else hashlib.sha256(os.getcwd().encode()).hexdigest()[:16]
            )
        except Exception:
            import hashlib
            ...
            if row:
                ...
                """,
                    (
                        ...
                        now,
                        repo_id,
                        repo_id,  # For the ON CONFLICT clause
                    ),
                )
...
def main():
    ...
    aggregator = CandidateAggregator(analyze_transcript=not args.no_transcript)
    ...
    if args.transcript_only:
        ...
        if aggregator.transcript_analyzer:
            ...
            if args.verbose:
                ...
                print(
                    f"Messages: {counts.get('user', 0)} user, {counts.get('assistant', 0)} assistant, {counts.get('tool_calls', 0)} tool calls"
                )
    ...
    if args.verbose:
        ...
        for c in candidates:
            ...
            print(
                f"  [{c['candidate_type']}] {c['title']} (confidence: {c['confidence']:.2f}){llm_tag}{transcript_tag}"
            )
